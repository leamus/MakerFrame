...
版本A.B.C说明：
    A表示大版本，一般情况下不会变化（如果变化表示鹰歌有极大更改，不会兼容以前工程）；
...
1、封装了Libhv的HTTPServer来实现简单的HTTP服务器；
2、增加了QWebApp库（后期再封装）；
3、增加了QextSerialPort串口库；
**4、将TapSDK更新到v4最新版，实名认证函数也有变化，且旧游戏需要Tap后台提工单升级SDK版本；
5、TapAD更新到最新版；
6、安卓端最新支持：能使用文件管理器选择鹰歌打开任何文件（可以作为万能播放器使用），QML文件默认直接运行；
7、安卓端最新支持：能使用Scheme URL（比如用链接来传递数据）；
8、安卓端最新支持：能分享到鹰歌来进一步处理；
9、Updater修复一个Bug；
10、Windows增加系统托盘图标；
11、其他：优化很多代码、调整很多小细节、修复一些Bugs；

2024/10/27：发布 1.13.9.241027 版本（框架 1.5.3.241027版本）
1、桌面端可拖动QML文件到鹰歌来新建窗口运行，可以运行一些额外的小扩展、软件来辅助开发；
2、对C++的TCP和UDP对象进行低级封装为 QML的 TCPServer、TCPSocket、UDPSocket 3个组件，实现TCP、UDP协议的服务端和客户端；
3、修复软件退出时奔溃Bug（QML释放了Qt对象，原因未知）；
