...
108、新增：插件$init函数增加参数loadResources，可以用此来优化插件加载速度（比如Pymo和BBKRPG插件）；
109、修复：修复一些焦点问题，和因为焦点导致输入法乱跳的问题；
110、修复：不规则插件名会导致无法安装插件的Bug；
111、新增：建议功能；
112、新增：安装Qt插件后自动刷新载入插件；
113、修复：绕过QML的一个BUG：安卓下使用 import xxx 会导致 assets:/ 的路径多了前缀 file: 导致打包后无法启动游戏；
114、优化：request函数；
115、修复：一些焦点问题（影响各个编辑器）；
116、修复：窗口大小有时不正确的Bug；
117、修改：为适配OpenHarmony修改、修复了很多路径问题；
118、新增：OpenHarmony获取存储权限；
119、修复：sl_toURL、sl_toPath的返回不正确Bug；
120、修复：访问HTTP如果错误会导致闪退的BUG（sender()的坑，安卓上不知为何不会出现）；
121、修改：为适配OpenHarmony修改、修复了很多屏幕适配问题；
122、修复：无法删除老旧插件的Bug；
123、新增：QJSEngine、QQMLEngine、QQMLApplicationEngine的相关函数；
124、新增：$showToast全局函数；
125、修改：为打包APP时插件的问题做了适配；
126、新增：日历组件；
...
160、修改：融合APP和游戏打包环境；
161、修复：Extends的焦点等诸多Bugs；
162、修复：应用退出释放插件时奔溃的Bug；
163、新增：sl_counter计数器函数；
164、新增：安卓下一些root命令相关函数；
165、修改：win下还原了默认字符集，更改quazip文件名默认字符集为utf8；
166、增强：ScriptEditor功能，支持运行qml、js、json；
167、修复：绕过QML的一个alias引用js导入名的Bug；
168、其他：优化调整很多代码和细节，修复一些Bugs；
