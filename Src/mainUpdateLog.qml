...
**10、将之前改的 game.$sys.init、game.loadmap、game.usegoods、game.load、game.gameover、game.plugin、game.save 命令改为前面必须加 yield *，这样解决了各种莫名其妙的问题（因为 game.run 参数为-2立即执行，中断后再次恢复时，传递的值可能不是执行命令后返回的值）；
11、可视化指令相应调整；
12、其他Bug修复；
