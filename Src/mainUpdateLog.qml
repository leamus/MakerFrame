...
*1、将 game.save、game.usegoods、game.over、game.plugin 做成同步执行了（前加yield），也尽力兼容了异步执行（只有一种情况可能会产生逻辑错误：如果你命令前不写yield，则会导致你这条命令并没有完全执行完毕（如果含有异步指令）又去执行余下的代码，然后又去返回执行命令里异步代码 产生顺序错误）；
