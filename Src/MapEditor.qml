...
/*
  ...
  导出：
    ...
    EventData：
      所有事件名和事件的定义。
      createEvent时，会跳过 $ 开头的事件名。$开头的事件名是系统功能。
...
Rectangle {
    ...
    function createNewMap(cfg) {
        ...
        imageMapBlock1.source = Global.toQMLPath(GameMakerGlobal.mapResourceURL(cfg.MapBlockImage[0]));
    ...
    function openMap(cfg) {
        ...
        //读取事件数据，创建事件
        for(let i in cfg.EventData) {
            //跳过 $ 开头的事件名
            if(i.indexOf("$") === 0)
                continue;

            _private.createEvent(i, cfg.EventData[i]);
        }
    ...
    function exportMap(path) {
        ...
        //绘制所有 0层及以上 canvas
        canvasOutput.strOutputPath = path;
        ...
        let ret = GameManager.sl_qml_WriteFile(JSON.stringify(outputData), path + Platform.separator() + 'map.json', 0);
        //console.debug(canvasMapContainer.arrCanvasMap[2].toDataURL())

        console.debug("[MapEditor]exportMap ret:", ret, path + Platform.separator() + 'map.json');
            ...
            ListView {
                ...
                delegate: ItemDelegate {
                    ...
                    Label {
                        ...
                        font.strikeout: canvasMapContainer.arrCanvasMap[index] ? (canvasMapContainer.arrCanvasMap[index].visible ? false : true) : false
                        text: {
                            switch(index) {
                            //case 0:
                                //return "设置障碍";
        ...
        RowLayout {
            ...
            ColorButton {
                ...
                onButtonClicked: {
                    ...
                    GameManager.setPlainText(textCode.textDocument, objSystemEventsData["$1"] || "");
                    dialogEvent.nEventIndex = -2;
                    dialogEvent.open();
                }
            }
                ...
                delegate: ItemDelegate {
                    ...
                    MouseArea {
                        ...
                        onDoubleClicked: {
                            ...
                            GameManager.setPlainText(textCode.textDocument, objEventsData[textEventName.text]);
                            dialogEvent.open();
                            //canvasMapContainer.arrCanvasMap[index].visible = bVisible;
                        }
                    }
            ...
            ColorButton {
                ...
                onButtonClicked: {
                    ...
                    loaderTestMap.item.focus = true;
                    loaderTestMap.focus = true;
                    loaderTestMap.visible = true;
                }
            }
...
//地图块区域
    ...
    Dialog {
        ...
        onAccepted: {
            ...
            exportMap(GameMakerGlobal.config.strProjectPath + Platform.separator() + GameMakerGlobal.config.strCurrentProjectName + Platform.separator() + GameMakerGlobal.config.strMapDirName + Platform.separator() + _private.strMapName);
    ...
    Dialog {
        ...
        //参数；要修改的EventIndex，-1表示新建；-2表示系统事件（不写入listview，只写入SystemEvents）
        property int nEventIndex: -1

        visible: false
        title: "请输入事件脚本（载入外部脚本用game.script）"
        ...
        onAccepted: {
            ...
            if(nEventIndex === -1) {    //新建事件
                ...
                _private.createEvent(textEventName.text, GameManager.toPlainText(textCode.textDocument));

            }
            else if(nEventIndex === -2)  {  //新建系统事件
                //objSystemEventsData[textEventName.text] = textCode.text;
                objSystemEventsData[textEventName.text] = GameManager.toPlainText(textCode.textDocument);
            }
                ...
                objEventsData[textEventName.text] = GameManager.toPlainText(textCode.textDocument);
    ...
    */
    ...
    Canvas {
        ...
        onPaint: {
            if(strOutputPath !== "") {
                ...
                canvasOutput.save(strOutputPath.replace("file:/", "").replace("//", "") + Platform.separator() + "map.png");
                //GameManager.sl_qml_WriteFile(canvasOutput.toDataURL("image/png"), strOutputPath + '/output_map.png', 0);
    ...
    QtObject {
        ...
        //创建事件（只是给listview加）
        function createEvent(name, code) {
