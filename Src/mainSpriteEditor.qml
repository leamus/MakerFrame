...
Item {
    ...
    ColumnLayout {
        ...
        L_List {
            ...
            onSg_removeClicked: {
                const dirPath = $GameMakerGlobal.spritePath(item);
                console.debug('[mainSpriteEditor]删除：', dirPath, $Frame.sl_dirExists(dirPath), );
    ...
    QtObject {
        ...
        function refresh() {
            const list = $Frame.sl_dirList($GameMakerGlobal.spritePath(), [], 0x001 | 0x2000 | 0x4000, 0x0);
            //list.unshift('【新建特效】');
