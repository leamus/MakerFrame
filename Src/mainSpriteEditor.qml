...
Rectangle {
    ...
    ColumnLayout {
        ...
        ColorButton {
            ...
            onButtonClicked: {
                ...
                dirList.focus = true;
                dirList.showList(GameMakerGlobal.config.strProjectPath + Platform.separator() + GameMakerGlobal.config.strCurrentProjectName + Platform.separator() + GameMakerGlobal.config.strSpriteDirName, "*", 0x001 | 0x2000, 0x00);
    ...
    DirList {
        ...
        onRemoveClicked: {
            let dirUrl = GameMakerGlobal.config.strProjectPath + Platform.separator() + GameMakerGlobal.config.strCurrentProjectName + Platform.separator() + GameMakerGlobal.config.strSpriteDirName + Platform.separator() + item;

            dialogCommon.text = "确认删除?";
            dialogCommon.fOnAccepted = ()=>{
                console.debug("[mainSpriteEditor]删除：" + dirUrl, Qt.resolvedUrl(dirUrl), GameManager.sl_qml_DirExists(dirUrl), GameManager.sl_qml_RemoveRecursively(dirUrl));
                removeItem(index);
    ...
    */
    Dialog {
        ...
        onAccepted: {
            ...
            console.debug("[mainSpriteEditor]onAccepted");
        }
        onRejected: {
            if(fOnRejected)fOnRejected();
            console.debug("[mainSpriteEditor]onRejected");
        }
    }
