...
')
    ...
    QtObject {
        ...
        function compile(checkResult=true) {
            ...
            case 0:
                type = '0';
                checkScript = _private.strTemplate1checkScript;
            ...
            case 1:
                type = '1';
                checkScript = _private.strTemplate2checkScript;
                checkScript = $CommonLibJS.replaceAll(checkScript, '$$MP$$', requiredMP);
            ...
            //全体
            if(targetCount === '-1') {
                buffs = $CommonLibJS.replaceAll(strTemplateBuffs, '$$buffs$$', buffs);
            }
            //单体
            else {
            ...
            /*data = $CommonLibJS.replaceAll(data, '$$checkScript$$', textLuck.text.trim());
            ...
            */
            data = $CommonLibJS.replaceAll(data, '$$checkScript$$', checkScript);
...
`
        //不用检查
        property string strTemplate1checkScript: `
            return true;
`

        //检查MP
        property string strTemplate2checkScript: `
...
`
        ...
        //技能全体Buffs
        property string strTemplateBuffs: `
            //每个被攻击Buffs
            for(let ti in targetCombatants) {
                let targetCombatant = targetCombatants[ti];
                if(targetCombatant.$$propertiesWithExtra.HP[0] <= 0)
