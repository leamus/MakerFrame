...
<CENTER><B>关　于</B></CENTER>
  ...
  作者的话
1.鹰歌MakerFrame框架底层已经完善，游戏引擎目前完成了 RPG/ARPG/文字AVG/放置/策略与战旗 等类型，其功能、扩展性、自由度都非常不错，还提供了完整可扩展的图形化编程和各类图形化编辑器，但很多细节和功能仍然正在完善和优化（由于时间精力和经济问题导致UI和操作体验不太完美），同时也欢迎有志同道合的朋友能参与一起开发。
...
6.各平台的框架引擎都支持热更新（打开软件后会自动检测并更新 内核动态链接库、引擎QML和Java代码库Dex），无需重新下载安装新版（服务器带宽较慢，可能升级和下载时间会稍微长一点）。

  功能和特色：
1.跨平台：框架、引擎和游戏都可完美运行在Windows（win7及以上）、安卓（6.0及以上）、macOS、iOS、Linux（包括Ubuntu、Debian、国产化的OpenKylin和UOS、Arm的树莓派）等操作系统上；
...
4.多种发布形式：能生成对应平台的安装包exe、apk、bin、deb等（可发布在Steam、Tap、OpenKylin等平台），也能生成框架能载入运行的游戏ROM包；资源和代码支持源文件形式，也支持压缩打包加密形式，防逆向盗取；ROM可以分发到各平台，或上传到官网，用链接、二维码、分享等形式来载入运行；
5.框架热更新：框架底层采用我编写的 升级加载运行器，能运行时无感自动热更新框架引擎的所有核心库（so/dll、dex、rcc），不用繁琐的下载和重新安装；
6.屏幕自适应：在任何分辨率，各种不同大小的屏幕下有相同的显示效果；开发时引擎还提供了不同的方式来解决各种屏幕的自适应效果（比如简单的按比例缩放、虚拟坐标、点坐标、布局方式等）；
...
3.此框架引擎的重点在于打造 手持端（兼容PC、MAC等跨端）和国产化系统 的跨平台游戏框架引擎，专业的还请绕道Unity、虚幻等更好的引擎；
4.未来：
