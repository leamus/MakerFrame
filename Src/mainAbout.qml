...
<CENTER><B>关　于</B></CENTER>
  ...
  简介
1.鹰歌MakerFrame软件框架 是一款由 Qt（C++）、QML（JavaScript）、JAVA（Android） 构建，供二次开发软件和游戏的开放式跨平台框架，它可以运行在Windows、Android、iOS、macOS、Linux（Debian、Ubuntu以及国产化的Openkylin、UOS）等主流平台上，并提供了上架应用商店、广告接口、网络、资源整合打包、屏幕自适应、热更新、压缩解压、文件下载、数据库、文件管理、线程池、脚本引擎、异步脚本（协程）、脚本/事件队列、项目打包生成、平台分发上架、媒体播放器、浏览器内核、以及各种三方库和SDK等丰富的底层功能和扩展，能满足多种类型的引擎、游戏及各种软件开发的一款超级APP；
2.鹰歌游戏引擎 是基于 鹰歌MakerFrame软件框架 开发的一套开放式（支持RPG、ARPG、策略与战旗、文字AVG、放置类等几乎所有2D类型）游戏开发引擎和运行环境，主要由QML（JavaScript）编写，支持图形化编辑器（如 地图编辑器、角色编辑器、特效编辑器、道具编辑器、技能编辑器、战斗人物编辑器、战斗脚本编辑器、升级链编辑器、图片音乐视频管理、图形化脚本编程等）、代码或两者结合的方式来设计游戏和插件；

  QQ群：654876441


  作者的话
1.鹰歌MakerFrame软件框架底层已经完善，游戏引擎目前完成了 RPG/ARPG/文字AVG/放置/策略与战旗 等类型，其功能、扩展性、自由度都非常不错，还提供了完整可扩展的图形化编程和各类图形化编辑器，但很多细节和功能仍然正在完善和优化（由于时间精力和经济问题导致UI和操作体验不太完美），同时也欢迎有志同道合的朋友能参与一起开发。
2.鹰歌游戏引擎有三种方式来开发设计（其实并不局限于这三种方式，因为它的用法是针对每种类型每个对象都可以不同，比如你某个道具可以用脚本，另一个道具可以用图形化编辑，其他类型和对象也是如此，所以自由度非常高）：
  a、纯图形化编辑，这种是最简单，最快速的方法（为照顾小白），缺点是功能固化，模式单一；
  b、纯写脚本，引擎只需会JavaScript（有一点QML基础更好）就行，优点是开放度、自由度、功能性非常高，技术上不封顶，缺点是比图形化稍难一些（适合大神）。
...
2.全功能网络：支持 TCP（Qt）、UDP（Qt）、HTTP（XmlHttpRequest和QNetwork封装的两种方式，后者自由度高）、Websocket（QML）等多种常用协议，也可自己封装其他协议；API使用非常简单，支持异步函数的同步写法；支持互联网、局域网、蓝牙、NFC等通信方式；
...
4.多种发布形式：能生成对应平台的安装包exe、apk、bin、deb等（可发布在Steam、Tap、OpenKylin等平台），也能生成框架引擎能载入运行的游戏ROM包；资源和代码支持源文件形式，也支持压缩打包加密形式，防逆向盗取；ROM可以分发到各平台，或上传到官网，用链接、二维码、分享等形式来载入运行；
...
15.可开发类型：
  ...
  软件：鹰歌框架引擎封装了很多基础和底层的功能（文件、网络等等），加上QML的易用性，所以也很适合开发各种行业应用软件、系统软件、甚至包括爬虫；
  其他：还可以增强鹰歌本身的功能，比如用QML的插件机制来编写QML扩展、编写QML组件、游戏插件，用本地语言来调用系统功能等等；
...
1.功能分层：
  ...
  c.底层（QML+JavaScript）：用来扩展界面和功能，也可使用框架提供的Box2D和qnanopainter库，比如制作插件、扩展图形化命令、修改增强游戏引擎和编辑器、甚至可以做其他类型的游戏引擎、编辑器和游戏，还可以做各种类型的软件APP（系统软件、行业业务软件、播放器、浏览器、爬虫等）；
...
4.未来：
  ...
  b.优化引擎：引入缓存机制，并将QML中效率不高的图形引擎替换为OpenGL，JavaScript算法替换为C++等优化；
  c.开发3D引擎（已引入QML3D）；
