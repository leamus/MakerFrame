...
<CENTER><B>关于</B></CENTER>
  ...
  鹰歌RPG Maker引擎 是基于 鹰歌MakerFrame框架 开发的一套开放式 RPG/文字/放置类 游戏制作引擎和运行环境，使用QML/JS编写，适用于开发游戏和插件，可使用视图编程、代码或两者结合的方式；

  QQ群：654876441


  作者的话
鹰歌MakerFrame和RPG Maker引擎 的完成度还可以（除了UI，由于时间精力经济问题），框架基本完成，游戏引擎只有RPG类型（兼容放置类），目前已基本完善，除了界面丑陋，体验有点差以外，功能、扩展、自由度都还不错，视图编程和视图编辑器也已经完成，很多细节和功能也正在完善和优化，同时也希望有人能参与一起完善框架和引擎。
