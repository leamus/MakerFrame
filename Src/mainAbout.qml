...
<CENTER><B>关　于</B></CENTER>
  ...
  鹰歌MakerFrame框架 是一款由 Qt/QML/JAVA 构建的供二次开发的软件/游戏的制作及运行的开放式跨平台框架，它可以运行在Windows、Android、iOS、macOS、Linux（Ubuntu、包括国产化）等主流平台上，并提供了上架应用商店、广告、屏幕自适应、网络、热更新、数据库、文件管理、媒体播放器、浏览器内核等众多功能和扩展，是用来开发多类型游戏、引擎及软件的一款超级APP；
  鹰歌游戏引擎 是基于 鹰歌MakerFrame框架 开发的一套开放式 RPG/ARPG/策略与战旗/文字AVG/放置类 等游戏制作引擎和运行环境，主要由QML/JS编写，适用于开发游戏和插件，可使用图形化、代码或两者结合的方式来制作；

  QQ群：654876441


  作者的话
鹰歌MakerFrame框架和游戏引擎 的完成度还不错（除了UI不太完美，由于时间精力和经济上的问题），框架已经完成，游戏引擎目前完成了RPG/ARPG/策略与战旗/文字AVG/放置类，其功能、扩展、自由度都非常不错，还提供了完整可扩展的图形化编程和图形化编辑器，很多细节和功能仍然正在完善和优化，同时也欢迎有志同道合的朋友能参与一起完善框架和引擎。
目前用鹰歌有三种方式来制作游戏（其实并不局限于这三种方式，因为它的用法是针对每种类型每个对象都可以不同，比如你某个道具可以用脚本，另一个道具可以用图形化编辑，技能之间也可以不同，自由度非常高）：
    1、纯图形化编辑，这种是最简单，最快速的方法，缺点是功能固化，模式单一；
    2、纯写脚本，引擎只需会JavaScript（有一点QML基础更好）就行，优点是开放度、自由度、功能性非常高，技术上不封顶，缺点是稍微稍难一点。
    3、图形化编辑+脚本混合，我非常推荐的一种方法，可以逐渐深入引擎并编写各种功能和玩法，方法是某个对象（道具、技能或人物等）可以用图形化编辑生成一个代码模板，然后在此基础上修改，但注意某个图形化编辑对象一旦重新编辑编译，会替换你对应改好的脚本（其实生成后不用再动图形化编辑，而且对象粒度很小）。
...
5. 各平台软件都是热更新（内核、引擎和Java代码），大部分情况下无需额外操作（服务器带宽较慢，可能升级和下载时间会稍微长一点）。
  ...
  功能和特色：
1.跨平台：框架、编辑器和游戏都可完美运行在win、安卓、macos、ios、linux（包括x86、arm的Ubuntu、国产化UOS、麒麟、树莓派）等系统平台；
2.可联网：框架有后台服务器和数据库，可做注册登录、聊天、房间、联机对战等功能，且都是跨平台共用的；
3.多种发布形式：能生成对应平台的安装包exe、apk、bin等（可发布在steam、tap等平台），也可生成框架可载入的游戏资源ROM；资源和代码可原样提供，也可压缩打包，防逆向盗取；ROM可分发到各平台，或上传到官网，用链接、二维码、分享等形式来载入运行）；
4.框架热更新：框架底层采用我编写的升级载入运行器，可对C++编写的库、Java编写的安卓库、QML编写的编辑器和游戏，都可无感自动热更新，不用繁琐的下载和重新安装；
5.屏幕自适应：在任何分辨率，各种不同大小的屏幕下有相同的显示效果；引擎还提供了不同的方式来应对各种屏幕（比如简单的按比例缩放、虚拟坐标、固定size显示、布局方式等）；
6.多层次的架构设计，可满足和适应不同技术的玩家（见 架构设计）；
7.几行代码便可接入 穿山甲和Tap 广告；
8.其他方面/技术：
  ...
  以上功能大都已封装为QML或js扩展来使用，还可以用QML插件机制来编写扩展；
...
5、功能分层：
    a、上层（纯图形化编辑开发）：道具、战斗人物、技能、战斗脚本使用图形化编辑生成，脚本使用纯图形化编程来制作，只需设计 剧情、道具、地图、人物、战斗 等等就可以做出游戏，后期还可能做Excel、Json等来设计游戏内容，框架导入来生成游戏；
    b、中层（图形化编辑+JS代码）：用JS代码来扩展图形化编辑，可以开发和设计更多的内容，比如游戏的算法、界面效果、升级链、道具、人物、登录、联网功能、图形化命令 等等；
    c、底层（QML+JS）：用QML/JS语言来扩展界面和功能，也可使用框架提供的Box2D和qnanopainter库，比如制作插件、扩展图形化命令、修改引擎和编辑器、甚至可以做其他类型的游戏引擎、编辑器和游戏，还可以做各种类型的软件APP（系统软件、业务软件、播放器、浏览器等）；
