...
<CENTER><B>关于</B></CENTER>
  ...
  鹰歌MakerFrame框架 是一款由 Qt/QML/JAVA 构建的供二次开发的软件/游戏的制作及运行的开放式跨平台框架，它可以运行在Windows、安卓、苹果、Linux（包括国产化）等主流平台上，并提供了屏幕自适应、网络、热更新、数据库、文件管理、媒体播放器、浏览器内核等众多功能和扩展，可以用来开发很多种软件、游戏和引擎等；
  鹰歌RPG Maker引擎 是基于 鹰歌MakerFrame框架 开发的一套开放式 RPG/文字/放置类 游戏制作引擎和运行环境，使用QML/JS编写，适用于开发游戏和插件，可使用视图编辑、代码或两者结合的方式；

  QQ群：654876441


  作者的话
鹰歌MakerFrame和RPG Maker引擎 的完成度还可以（除了UI，由于时间精力经济问题），框架基本完成，游戏引擎只有RPG类型（兼容放置类），目前已基本完善，除了界面丑陋，体验有点差以外，功能、扩展、自由度都还不错，视图编辑器也已经完成，很多细节和功能也正在完善和优化，同时也希望有人能参与一起完善框架和引擎。
...
3、此框架引擎重点在于打造手持端（兼容PC、MAC等跨端）和国产化系统的游戏框架引擎，专业的还请绕道Unity、虚幻等更好的引擎；
4、此框架适合：
...
5、功能分层：
    a、上层（纯视图编辑开发）：脚本使用纯视图编辑来制作，只需设计 剧情、道具、地图、人物、战斗 等等就可以做出游戏，后期还可能做Excel、Json等来设计游戏内容，框架导入来生成游戏；
