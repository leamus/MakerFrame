...
简要说明：
  1、创作对象基本分为三大部分：地图场景（包括地图、角色、特效）、战斗场景（道具、战斗角色、技能、战斗脚本）和脚本（起始脚本、通用脚本），每部分都有对应的帮助；
    通用脚本包含了各种功能和配置，可以用默认的，如果有代码基础可以自行修改和编写；
    战斗角色和战斗技能的特效，都是来源于 “特效”对象；

  2、创作步骤：一般是：
    地图行走游戏：地图->角色->起始脚本；
    战斗：特效（人物）->技能->战斗角色->战斗脚本；


<font color='red'>可视化版本</font>

  如果被脚本吓住了，那直接忽略它吧，因为鹰歌支持可视化编辑和可视化编程，编辑器界面和脚本界面一般都有个 V 按钮（Visual可视化的意思），点击就可以进行可视化操作了。编辑后切记要点保存（保存可视化数据）和编译（生成代码），然后返回到代码界面再保存一次即可。
    可视化的文本处，有@标记的都是可以长按选择，有*的是必填项；
  可视化版本优点是简单，缺点是繁杂，模式固定；脚本恰好相反。JS脚本也不是很难，而且可以做出很多效果，所以新手推荐先使用可视化，生成一个简单的游戏后研究下脚本代码，试着修改和编写，循序渐进。老手可以直接干代码。
...
脚本变量：
  ...
  支持JS的 let、var、const来定义变量，也可以用下面引擎提供的特殊对象来存储数据：
  game.d：用户可自定义 地图变量 对象（切换地图后清空）。
  game.f：用户可自定义 地图函数 对象（切换地图后清空）；与game.d不同的是地图脚本的数据会拷贝在内。
  game.gd：用户可自定义 全局变量 对象（一个游戏进度可用，且会写入存档）。
  game.gf：用户可自定义 全局函数 对象（一个游戏进度可用）；game.js脚本的数据也会拷贝在内。
  game.cd：用户可定义 游戏引擎变量 对象，整个游戏可用，进入游戏引擎时会读取，退出游戏时保存，读档不会清空；
  game.cf：用户可自定义 游戏引擎函数 对象，整个游戏可用，游戏退出后清空；
  game.http：新建一个 XMLHttpRequest对象。
  game.date(format, date)：按格式返回日期。
