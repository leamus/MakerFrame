                ...
                onClicked: {
                    $dialog.show({
                        ...
                        OnAccepted: function() {
                            ...
                            const failed = [];
                            const noCompile = [];
                            const list = $Frame.sl_dirList($GameMakerGlobal.fightScriptPath(), [], 0x001 | 0x2000 | 0x4000, 0x0);
                            for(const tn of list) {
                                const path = $GameMakerGlobal.fightScriptPath(tn) + '/';
                                if(!$Frame.sl_fileExists(path + 'fight_script.vjs')) {
                                    //console.debug('[mainFightScriptEditor]没有可视化文件:', tn);
                                    noCompile.push(tn);
                                    continue;
                                }
                            ...
                            console.info('[mainFightScriptEditor]编译成功：%1，失败：%2，没有编译：%3'.arg(successed).arg(failed).arg(noCompile));
                        },
                        OnRejected: ()=>{
                            //l_listFightScript.forceActiveFocus();
                        },
        ...
        onLoaded: {
            ...
            /*if(item.$load) {
                try {
                    ...
                    //console.warn('[!mainFightScriptEditor]', e);
                    //throw e;
                }
                finally {
                }
